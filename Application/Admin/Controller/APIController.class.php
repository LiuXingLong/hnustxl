...
class APIController extends Controller{
    public function index(){
    	if($_GET['Key']!="c3f51dfad7ed75231bbbc3281888c9bc"){
    		echo "您无权使用！";
    		return 0;
    	}
    	...
    	if($_SESSION["'{$username}'"]==1){
    		$Grade=M('gradejz');
    		$Score=$Grade->where("username='%s'",$post["username"])->select();
	...
	public function yzm($Url){		
		...
		*/
		...
		//设置 session	
		session('cookieAPI',$cookieStr);
		session('vercodeAPI',$validCode); // "1234" $validCode		
		unlink($verPic);// 删除图片
	}	
	public function jwc($post,$Url){		
		...
		$postData = "xsxm=".$post["name"]."&xssfzh=".$post["card"]."&yzm=".session('vercodeAPI');		
		curl_setopt($ch, CURLOPT_POSTFIELDS, $postData);
		//curl_setopt($ch, CURLOPT_COOKIEFILE, $_COOKIE["cookie"]);
		curl_setopt($ch,CURLOPT_COOKIE,session('cookieAPI'));
		$login = curl_exec($ch);
